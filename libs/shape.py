...
class AlStartItem(object):
    def __init__(self,points: list,color: str="red",label=0):
        self.points = points  # 그릴 점들을 저장할 리스트
        self.label = label
        if color == "red":
            self.color = Qt.red
        elif color == "green":
    ...
    def paint(self, painter):
        radius = 5  # 그릴 원의 반지름 설정
        painter.setBrush(self.color)

        painter.drawEllipse(self.points, radius, radius)  # 각 점에 작은 원 그리기
