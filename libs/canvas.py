...
from libs.shape import Shape,AlStartItem
...
class Canvas(QWidget):
    ...
    CREATE, EDIT , SAM = list(range(3))

    epsilon = 24.0

    def __init__(self,parent=None,clas=None, *args, **kwargs):
    ...
    def mousePressEvent(self, ev):
        ...
        if self.mode is self.CREATE:
            if ev.button() == Qt.LeftButton:
                ...
                self.prev_point = pos
            self.update()
        elif self.mode is self.SAM:
            if ev.button() == Qt.LeftButton and int(pos.x()) > 0 and int(pos.y())>0:
                self.create_startItem("green",pos)
                self.set_hiding()
                self.drawingPolygon.emit(True)
                self.update()
            elif ev.button() == Qt.RightButton and int(pos.x()) > 0 and int(pos.y())>0:
                self.create_startItem("red",pos)
                self.set_hiding()
                self.drawingPolygon.emit(True)
                self.update()
    
    def create_startItem(self, color: str, pos) -> AlStartItem:
        point = QPointF(int(pos.x()),int(pos.y()))
        if color == "red":
            label = 0
        elif color =="green":
            label = 1
        start_item = AlStartItem(color=color,points=point,label=label)
        self.show_boxClass(color, start_item)
        return start_item

    def show_boxClass(self, classes: str, item) -> None:
        item.classes = classes

        list_item = QListWidgetItem()
        list_item.setData(Qt.UserRole, item)
        list_item.setText(classes)
        self.clas.addItem(list_item)
    ...
    def getItemsFromListWidget(self,list_widget):
        ...
        for i in range(list_widget.count()):
            ...
            # 'Qt.UserRole'을 사용하여 사용자 정의 데이터를 가져옵니다.
            item_data = list_item.data(Qt.UserRole)
            if item_data:
                points.append(item_data)
                label.append(item_data.label)
        return points, label

    def mouseReleaseEvent(self, ev):
        if self.mode is not self.SAM:
    ...
    def paintEvent(self, event):
        ...
        items ,label= self.getItemsFromListWidget(self.clas)
        if items:
            for item in items:
                item.paint(p)
    ...
    def keyPressEvent(self, ev):
        ...
        elif key == Qt.Key_Q and self.mode == self.CREATE:
            self.toggle_SAM()
        elif key == Qt.Key_Q and self.mode == self.SAM:
            self.toggle_SAM()
