...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        # 파일 디렉토리
        self.move_dir_img_path = None
        self.move_dir_label_path = None

        self.move_count = 0

        self.sam = SA(model_name="vit_h", model_path="sam_vit_h_4b8939.pth")
...
###############################
        self.SAM_list_container = QListWidget()
        SAM_list_layout = QVBoxLayout()
        SAM_list_layout.setContentsMargins(0, 0, 0, 0)
        SAM_list_layout.addWidget(self.SAM_list_container)
        SAM_list_container = QWidget()
        SAM_list_container.setLayout(SAM_list_layout)

        self.SAM_list = QDockWidget("SMA List", self)
        self.SAM_list.setObjectName("SMA List")
        self.SAM_list.setWidget(SAM_list_container)
        self.SAM_list_container.itemDoubleClicked.connect(self.file_item_double_clicked)
##################################
        ...
        self.canvas = Canvas(parent=self,clas=self.SAM_list_container)
        ...
        self.addDockWidget(Qt.RightDockWidgetArea, self.SAM_list)
        ...
        #####################
        move = action("move", self.move_data,
                                 'Return' , 'next', get_str('openDir'))
        
        move_dir_img = action("move Dir img", self.move_dir_img,
                                 'Ctrl+Shift+i', 'open', get_str('openDir'))
        
        move_dir_label = action("move Dir label", self.move_dir_label,
                                 'Ctrl+Shift+l', 'open', get_str('openDir'))
        predict_sam = action("predict SAM", self.predict_sam,
                                 'Shift+space' , 'new', None)
        #################################
        ...
        self.actions.beginner = (
            open, open_dir, change_save_dir,move,move_dir_img,move_dir_label,predict_sam, open_next_image, open_prev_image, verify, save, save_format, None, create, copy, delete, None,
    ...
    def file_item_double_clicked(self,item):
        row = self.SAM_list_container.row(item)
        self.SAM_list_container.takeItem(row)
        self.canvas.update()
...
#################################################
    def move_data(self):
        if self.move_dir_img_path is None or self.move_dir_label_path is None:
            return
        if not self.may_continue():
            return
        current_image_path = self.file_path
        img_filename_path = self.dir_name.replace("/","\\")
        filename, _ = os.path.splitext(current_image_path.replace(img_filename_path, ""))
        replace_file_path = self.default_save_dir.replace('/', '\\')
        txt_file_path = f"{replace_file_path}{filename}.txt"
        print(f"{current_image_path}\n{txt_file_path}")
        self.move_count +=1
        print(self.move_count)
        shutil.move(current_image_path, self.move_dir_img_path)
        shutil.move(txt_file_path, self.move_dir_label_path)
        self.m_img_list.pop(self.cur_img_idx)
        self.file_list_widget.takeItem(self.cur_img_idx)

    def move_dir_img(self, _value=False, dir_path=None, silent=False):
        if not self.may_continue():
            return

        default_open_dir_path = dir_path if dir_path else '.'
        if self.move_dir_img_path and os.path.exists(self.move_dir_img_path):
            default_open_dir_path = self.move_dir_img_path
        else:
            default_open_dir_path = os.path.dirname(self.file_path) if self.file_path else '.'
        if silent != True:
            target_dir_path = ustr(QFileDialog.getExistingDirectory(self,
            ...
            target_dir_path = ustr(default_open_dir_path)
        self.move_dir_img_path = target_dir_path
    def move_dir_label(self, _value=False, dir_path=None, silent=False):
        if not self.may_continue():
            return

        default_open_dir_path = dir_path if dir_path else '.'
        if self.move_dir_label_path and os.path.exists(self.move_dir_label_path):
            default_open_dir_path = self.move_dir_label_path
        else:
            default_open_dir_path = os.path.dirname(self.file_path) if self.file_path else '.'
        if silent != True:
            target_dir_path = ustr(QFileDialog.getExistingDirectory(self,
            ...
            target_dir_path = ustr(default_open_dir_path)
        self.move_dir_label_path = target_dir_path
    
    def change_point(self,points_list):
        points = []
    ...
    def predict_sam(self):
        ...
        x, y, w, h = self.sam.predict_box(image_path, points_list, labels)
        shape = Shape()
        shape.add_point(QPointF(x, y))
